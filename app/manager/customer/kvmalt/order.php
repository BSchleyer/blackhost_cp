...
if(isset($_POST['order'])){
    ...
    $db_price = $site->getProductOptionEntrie('2', $cores,'price')
    ...
    $usermail = $user->getDataById($userid,'username').$helper->generateRandomString('4')."@kvmhost.black-host.eu";
		...
		include BASE_PATH.'app/manager/customer/kvm/functions/createVm.php';
		sleep(4);

        $user->removeMoney($price, $userid);
        $user->addTransaction($userid,'-'.$price,$proname.' Bestellung');
		
        $discord->callWebhook('<@&874784920332017715> Soeben wurde ein neuer KVM-INTEL bestellt von '.$username);
