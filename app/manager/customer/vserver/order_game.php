...
if(isset($_POST['order'])){
    ...
    $SQL3 = $db->prepare("SELECT * FROM `vm_host_nodes` WHERE `active` = 'yes' AND `api_name` = 'GAME' ORDER BY `id` ASC LIMIT 1;");
    $SQL3->execute();
    if ($SQL3->rowCount() != 0) {
    ...
    if(empty($error)){
        ...
        $discord->callWebhook('Soeben wurde ein neuer OVH-Game vServer bestellt von '.$username);
