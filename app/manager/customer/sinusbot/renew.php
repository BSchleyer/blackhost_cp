...
if(isset($_POST['renew'])){
    ...
    if(empty($error)){
        ...
        $SQLGetServerInfos = $db->prepare("UPDATE `sinusbots` SET `expire_at` = :expire_at, `state` = 'active' WHERE `id` = :id");
        $SQLGetServerInfos -> execute(array(":expire_at" => $expire_at, ":id" => $id));

        $user->removeMoney($price, $userid);
        $user->addTransaction($userid,'-'.$price,'Sinusbot Verlängerung');

        echo sendSuccess('Deine SinusBot Instanz wurde verlängert');

        header('refresh:3;url='.$helper->url().'manage/sinusbot/'.$serverInfos['id']);
    } else {
