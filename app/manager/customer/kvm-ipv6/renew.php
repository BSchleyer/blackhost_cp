...
if(isset($_POST['renew'])){
    ...
    if(empty($error)){
        ...
        $SQLGetServerInfos = $db->prepare("UPDATE `kvm_ipv6` SET `expire_at` = :expire_at, `state` = 'active' WHERE `id` = :id");
        ...
        header('refresh:3;url='.$helper->url().'manage/kvm-ipv6/'.$serverInfos['id']);
