...
$SQLGetServerInfos = $db->prepare("SELECT * FROM `gameserver_mc` WHERE `id` = :id");
...
if(isset($_POST['renew'])){
    ...
    if(empty($error)){
        ...
        $SQLGetServerInfos = $db->prepare("UPDATE `gameserver_mc` SET `expire_at` = :expire_at, `state` = 'active' WHERE `id` = :id");
        ...
        header('refresh:3;url='.$helper->url().'manage/gameserver/mc/'.$serverInfos['id']);
    } else {
