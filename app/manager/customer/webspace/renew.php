...
$SQLGetServerInfos = $db->prepare("SELECT * FROM `webspace` WHERE `id` = :webid");
$SQLGetServerInfos ->execute(array(":webid" => $id));
$serverInfos = $SQLGetServerInfos -> fetch(PDO::FETCH_ASSOC);

$SQL10 = $db->prepare("SELECT * FROM `webspace_packs` WHERE `plesk_id` = :plan_id");
$SQL10->execute(array(":plan_id" => $serverInfos['plan_id']));
$packInfos = $SQL10->fetch(PDO::FETCH_ASSOC);
