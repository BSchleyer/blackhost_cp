...
if(isset($_POST['renew'])){
    ...
    if(empty($error)){
        ...
        $SQLGetServerInfos = $db->prepare("UPDATE `marktplatz` SET `expire_at` = :expire_at, `state` = 'active' WHERE `id` = :id");
        $SQLGetServerInfos -> execute(array(":expire_at" => $expire_at, ":id" => $id));

        $user->removeMoney($price, $userid);
        $user->addTransaction($userid,'-'.$price,'Marktplatz-Produk Verlängerung');

        echo sendSuccess('Dein Marktplatz-Produkt wurde verlängert');

        header('refresh:3;url='.$helper->url().'manage/marktplatz');
