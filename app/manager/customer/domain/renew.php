...
if(isset($_POST['renew'])){
    ...
    if(empty($error)){
        ...
        $SQLGetServerInfos = $db->prepare("UPDATE `domains` SET `expire_at` = :expire_at, `state` = 'active' WHERE `id` = :id");
        $SQLGetServerInfos -> execute(array(":expire_at" => $expire_at, ":id" => $id));

        $user->removeMoney($price, $userid);
        $user->addTransaction($userid,'-'.$price,'Domain Verlängerung');

        echo sendSuccess('Deine Domain wurde verlängert');

        header('refresh:3;url='.$helper->url().'manage/domain/'.$serverInfos['id']);
