<?php
$currPage = 'front_Sinusbot bestellen';
include BASE_PATH.'app/controller/PageController.php';
include BASE_PATH.'app/manager/customer/sinusbot/order.php';

for ($i = 0, $z = strlen($a = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz1234567890#')-1, $s = $a{rand(0,$z)}, $i = 1; $i != 20; $x = rand(0,$z), $s .= $a{$x}, $s = ($s{$i} == $s{$i-1} ? substr($s,0,-1) : $s), $i=strlen($s));

    // rabatt [START]
    $SQL50 = $db->prepare("SELECT * from `produkt_rabatt` WHERE `produkt` = 'sinusbot'");
    $SQL50->execute();
...
<div class="content d-flex flex-column flex-column-fluid" id="kt_content">
        ...
        <div class="text-center" style="margin-top: 1px; margin-bottom: 30px;">
            <h1 style="font-size: 70px;">Unsere <b style="color: <?= env('MAIN_COLOR'); ?>">Sinusbot</b> Instanzen</h1>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-9">
                    <div class="card" style="border-radius: 15px 10px;">
                        <div class="card-header"><h1>Deutschland, Sinusbot Instanz - <a style="color:<?= env(MAIN_COLOR); ?>">NEU</a></h1>
                    ...
                    <?php } ?>
						...
						<hr>
							...
							<i class="fa fa-microchip" style="color: <?= env('MAIN_COLOR'); ?>"></i>
							  </span> bis zu <b>1</b> Intel Xeon vKerne (Max. 3,50 Ghz)
							<br>

							<span class="svg-icon svg-icon-primary svg-icon-1x">
							<i class="fa fa-memory" style="color: <?= env('MAIN_COLOR'); ?>"></i>
							  </span> bis zu <b>200</b> MB DDR4 ECC RAM
							<br>
							...
							<i class="fa fa-shield-alt" style="color: <?= env('MAIN_COLOR'); ?>"></i>
							  </span> DDoS-Schutz + DDoS-Filter 
							<i class=" fa fa-question-circle" style="color: <?= env('MAIN_COLOR'); ?>" data-toggle="tooltip" title="" 
							   data-original-title="Wir bieten einen DDoS-Schutz inkl. Filter mit bis zu
													800 GBIT/s Schutz."></i>
							<br>
							...
							<i class="fa fa-robot" style="color: <?= env('MAIN_COLOR'); ?>"></i>
							  </span> 2 MusikBots (Eigene Lizenzen nutzbar) <span class="badge badge-success">NEU</span>
							<br>
                        ...
                        <div class="card-body">
                            <form method="post" id="orderForm">
                                ...
                                <select class="form-control" id="cores" name="cores">
                                    ...
                                    $SQL->execute(array(":option_id" => '40'));
                                ...
                                <select class="form-control" id="memory" name="memory">
                                    <?php
                                    $SQL = $db->prepare("SELECT * FROM `product_option_entries` WHERE `option_id` = :option_id");
                                    $SQL->execute(array(":option_id" => '41'));
                                    if ($SQL->rowCount() != 0) { while ($row = $SQL->fetch(PDO::FETCH_ASSOC)) { ?>
									<?php include 'option.php'; ?>
                                    <?php } } ?>
                                </select>
                                ...
                                <br>
                                <i class="fas fa-key" style="color: <?= env('MAIN_COLOR'); ?>"></i> 
								<b><label style="font-weight: bold;">Admin Passwort setzen</label></b>
                                <input class="form-control" name="password_sb" id="password_sb" value="<?= $s; ?>"></input>
                ...
                <script>
                    ...
                    function update()
                    {
                        sum = parseFloat($("#cores").find("option:selected").data("price"))
                            ...
                            +parseFloat($("#backups").find("option:selected").data("price"))
