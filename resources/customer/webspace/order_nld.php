<?php
$currPage = 'front_Webspace DE Bestellen';
    ...
    <div class="d-flex flex-column-fluid">
        <div class="container">
            <div class="row">
                ...
                $SQL = $db->prepare("SELECT * FROM `webspace_packs` WHERE `kat` = '1' AND `plesk_id` IS NOT NULL");
                $SQL->execute();
                if ($SQL->rowCount() != 0) { echo '<div class="text-center" style="margin-top: 50px; margin-bottom: 50px;">
        <h1 style="font-size: 70px;">Unsere <b style="color: #6254FE;">Plesk</b> Webspace</h1>
    </div>';
                                ...
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        ...
                                        <ul class="nav nav-pills" id="myTab" role="tablist">
                                            <li class="nav-item">
                                                <a class="nav-link active" id="custom-tab<?= $row['id']; ?>"
                                                   data-toggle="tab" href="#custom<?= $row['id']; ?>" role="tab"
                                                   style="color:white;" aria-controls="custom<?= $row['id']; ?>"
                                                   aria-selected="true">Eigene Domain</a>
                                            </li>
                                            <li class="nav-item">
                                                <a class="nav-link" id="subdomain-tab<?= $row['id']; ?>"
                                                   data-toggle="tab" href="#subdomain<?= $row['id']; ?>" role="tab"
                                                   style="color:white;" aria-controls="subdomain<?= $row['id']; ?>"
                                                   aria-selected="false">Subdomain von Uns</a>
                                            </li>
                                        </ul>
                                        ...
                                        <div class="tab-content" id="myTabContent">
                                            <div class="tab-pane fade show active" id="custom<?= $row['id']; ?>"
                                                 role="tabpanel" aria-labelledby="custom-tab<?= $row['id']; ?>">

                                                <form method="post">

                                                    <label>Domain</label>
                                                    <input class="form-control" name="domainName"
                                                           placeholder="deine-domain.de" required>

                                                    <br>
                                                    <label for="agb<?= $row['id']; ?>_1" class="checkbox noselect">
                                                        <input type="checkbox" name="agb" id="agb<?= $row['id']; ?>_1">
                                                        <span></span>
                                                        Ich habe die <a href="<?= $helper->url(); ?>agb">AGB</a> und <a
                                                                href="<?= $helper->url(); ?>datenschutz">Datenschutzerklärung</a>
                                                        gelesen und akzeptiere diese.
                                                    </label>
                                                    <label for="wiederruf<?= $row['id']; ?>_1"
                                                           class="checkbox noselect">
                                                        <input type="checkbox" name="wiederruf"
                                                               id="wiederruf<?= $row['id']; ?>_1">
                                                        <span></span>
                                                        Ich wünsche die vollständige Ausführung der Dienstleistung vor
                                                        Fristablauf des Widerufsrechts gemäß Fernabsatzgesetz. Die
                                                        automatische Einrichtung und Erbringung der Dienstleistung führt
                                                    ...
                                                    <button type="submit" name="order"
                                                            class="btn btn-outline-success text-uppercase font-weight-bolder px-15 py-3">
                                            ...
                                            <div class="tab-pane fade" id="subdomain<?= $row['id']; ?>" role="tabpanel"
                                                 aria-labelledby="subdomain-tab<?= $row['id']; ?>">

                                                <form method="post">

                                                    <label>Domain</label>
                                                    <input class="form-control" style="background-color: #292929;"
                                                           readonly name="domainName"
                                                           value="web<?= rand(0, 9) . rand(0, 9) . rand(0, 9) . '-' . $userid; ?>.<?= env('CUSTOM_WEBSPACE_SUBDOMAIN'); ?>"
                                                           required>
                                                    ...
                                                    <label for="wiederruf<?= $row['id']; ?>_2"
                                                           class="checkbox noselect">
                                                        <input type="checkbox" name="wiederruf"
                                                               id="wiederruf<?= $row['id']; ?>_2">
                                                        <span></span>
                                                        Ich wünsche die vollständige Ausführung der Dienstleistung vor
                                                        Fristablauf des Widerufsrechts gemäß Fernabsatzgesetz. Die
                                                        automatische Einrichtung und Erbringung der Dienstleistung führt
                            ...
                            <div class="card shadow mb-5">
                                <div class="card-header text-center">
                                    ...
                                    <?php } elseif ($row['price_old'] !== 0) { ?>
                                        <b><span style="font-size: 28px; color:red;"><?= $row['price']; ?>€</span>/ 30
                                            Tage</b>
                                        <br>
                                        ...
                                        <div class="mt-7" align="center">
                                            <?php if ($user->sessionExists($_COOKIE['session_token'])) { ?>
                                                <a href="#" data-toggle="modal"
                                                   ...
                                                   class="btn btn-outline-primary text-uppercase font-weight-bolder px-15 py-3">
            ...
            <div class="row">
                ...
                $SQL = $db->prepare("SELECT * FROM `webspace_packs` WHERE `kat` = '1' AND `keyhelp_id` IS NOT NULL");
                $SQL->execute();
                if ($SQL->rowCount() != 0) { echo '<div class="text-center" style="margin-top: 50px; margin-bottom: 50px;">
        <h1 style="font-size: 70px;">Unsere <b style="color: #6254FE;">KeyHelp</b> Webspace</h1>
    </div>';
                    while ($row = $SQL->fetch(PDO::FETCH_ASSOC)) {
                                        ...
                                        <div class="tab-content" id="myTabContent">
                                            <div class="tab-pane fade show active" id="custom<?= $row['id']; ?>"
                                                ...
                                                <form method="post">
                                                    ...
                                                    <input hidden value="<?= $row['keyhelp_id']; ?>" name="planName">
